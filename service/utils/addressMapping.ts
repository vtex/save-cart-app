...
const approximationAccepted = 0.7

const clearAddress = (value) => {
    return value.toLowerCase().replace(".", "").replace(",", "").trim()
}

const getCityAndCep = (cities, city) => {
    for (const key in cities) {        
        if (stringSimilarity.compareTwoStrings(clearAddress(city), clearAddress(key)) > approximationAccepted) {
            return {
                city: key,
                cep: cities[key]
            }
...
export function getAddress(state, city, postalCode, countryCode) {
    if (countryCode != 'COL') {
        return {
            state: state,
            city: city,
    ...
    for (const key in states) {
        if (stringSimilarity.compareTwoStrings(clearAddress(state), clearAddress(key)) > approximationAccepted) {
            const citiesOfState = states[key]
            const cityAndCep = getCityAndCep(citiesOfState, city)

            return {
                state: key,
                city: cityAndCep.city,
                postalCode: cityAndCep.cep
            }
        }
    }
